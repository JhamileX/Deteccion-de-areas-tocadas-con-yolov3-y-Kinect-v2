...
void CDepthBasics::ProcessDepth(INT64 nTime, const UINT16* pBuffer, int nWidth, int nHeight, USHORT nMinDepth, USHORT nMaxDepth)
{
    ...
    if (m_pDepthRGBX && pBuffer && (nWidth == cDepthWidth) && (nHeight == cDepthHeight))
    {
        ...
        while (pBuffer < pBufferEnd)
        {
            ...
            nMaxDepth = 1500;
            int L=0;
            double hue = (depth) * (360.0) / nMaxDepth;
            double aux = hue - int(hue);
            double saturation = aux*50;
            double lightness = hue * 50.0/ 360.0;
            /*double saturation = int(hue) % 40;
            double lightness = hue * 100 / 360;
            int val = 1 + (int(depth - nMinDepth) % 40);
            if ((int(depth - nMinDepth)%1000)%2 == 0 ) {
                saturation = val;
            }
            else {
                saturation = 40 - val;
            ...
            HSVtoRGB(hue,50+ lightness,50+ lightness);
            int c = depth * 5041;
            BYTE intensity = static_cast<BYTE>((depth >= nMinDepth) && (depth <= nMaxDepth) ? (depth % 256) : 0);

            pRGBX->rgbRed   = static_cast<BYTE>((depth >= nMinDepth) && (depth <= nMaxDepth) ? xR : 0);
            pRGBX->rgbGreen = static_cast<BYTE>((depth >= nMinDepth) && (depth <= nMaxDepth) ? xG : 0);
            pRGBX->rgbBlue  = static_cast<BYTE>((depth >= nMinDepth) && (depth <= nMaxDepth) ? xB : 0);
           
            /*pRGBX->rgbRed = (depth & 0xff0000) >> 16;
            pRGBX->rgbGreen = (depth & 0x00ff00) >> 8;
            pRGBX->rgbBlue  = (depth & 0x0000ff);*/
